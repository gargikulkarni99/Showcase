...
def mentor_signup_form_submission(request):
    if request.method == 'POST':
        ...
        mentor.set_password(password)  # This will hash the password correctly
        mentor.save()

        return render(request, 'mentor-signup-summary.html', {
            'first_name': first_name,
            'last_name': last_name,
            'date_of_birth': date_of_birth,
            'phone_number': phone_number,
        ...
        return redirect('mentor-login.html', {'error': "Could not verify login"})
...
def mentor_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            # Log the user in
            login(request, user)
            return redirect('mentor_login_successful')
        else:
            return render(request, 'mentor-login.html', {'error': 'Invalid email or password.'})

    return render(request, 'mentor-login.html')

def mentor_forgotPassword(request):
    return render(request, 'forgot-password-mentor.html')
...
def mentor_resetPassword(request):
    if request.method == 'POST':
        ...
        request.session['email'] = email
        # Render the next page with the data
        return render(request, 'forgot-password-mentor-1.html', {'email': email})
    else:
        # Redirect or handle non-POST requests
        return redirect(request, 'forgot-password-mentor-1.html',{'error': 'Invalid email or password.'})
...
def mentor_resetPassword_Submission(request):
    if request.method == 'POST':
        ...
        if new_password != confirm_password:
            return render(request, 'forgot-password-mentor-1.html', {'error': 'Passwords do not match.'})
        ...
        try:
            ...
            return render(request, 'forgot-password-mentor-1.html', {'error': 'mentor not found. Please try again.'})
